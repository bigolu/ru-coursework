public class CellSimImproved{
	...
	for(int sim = 0;sim<101;sim++){
	    ...
	    */
		...
		if (sim==100){
			System.out.print("algo1 rounds: " + roundsNeeded1);
			System.out.println();
			System.out.print("algo2 rounds: " + roundsNeeded2);
		}
		
		algo1: while (true){
			if (boardSatisfied(tissue, threshold) || (numRounds1 >= maxRounds)){ //if board is satisfied or maxrounds is met, the simulation ends
				/*satisfied = boardSatisfied(tissue, threshold);
				endSimulation(initial, tissue, satisfied, numRounds, numMoved, totalNumMoved);*/
				roundsNeeded1 = roundsNeeded1 + numRounds1;
				numRounds1 = 0;
				break algo1;
			...
			numMoved = moveAllUnsatisfied(tissue, threshold); //moves unsatisfied and adds the number moved to total movements
			totalNumMoved += numMoved;                        //so I can print total movements at the end
			numRounds1++;
		}
		
		algo2: while (true){
			if (boardSatisfied(initial, threshold) || (numRounds2 >= maxRounds)){ //if board is satisfied or maxrounds is met, the simulation ends
				/*satisfied = boardSatisfied(tissue, threshold);
				endSimulation(initial, tissue, satisfied, numRounds, numMoved, totalNumMoved);*/
				roundsNeeded2 = roundsNeeded2 + numRounds2;
				numRounds2 = 0;
				break algo2;
			}
			...
			numMoved = moveAllUnsatisfied2(initial, threshold); //moves unsatisfied and adds the number moved to total movements
			totalNumMoved += numMoved;                        //so I can print total movements at the end
			numRounds2++;
		}
	...
	**/
	...
	/**
	    ...
	    **/
	    public static int moveAllUnsatisfied(char[][] tissue, int threshold){
	    	...
	    	for(int i = 0;i<tissue.length;i++){ //I made a boolean array of size nxn to tell if a cell is unsatisfied or not
	    		for(int j = 0;j<tissue[0].length;j++){
	    			if (isSatisfied(tissue, i, j, threshold) != true)
	    				unsatisfied[i][j] = true;
	    		}
	    	}
	    	
	    	for(int i = 0;i<tissue.length;i++){ //If a cell is unsatisfied, I check through the array for a blank spot and swap 
	    		for(int j = 0;j<tissue[0].length;j++){//the blank spot's value with the unsatisfied cell
	    			if (unsatisfied[i][j]){
	    ...
	    public static int moveAllUnsatisfied2(char[][] initial, int threshold){
	    	
	    	boolean[][] unsatisfied = new boolean[initial.length][initial[0].length];
	    	boolean[][] optimalX = new boolean[initial.length][initial[0].length];
	    	boolean[][] optimalO = new boolean[initial.length][initial[0].length];
	    	int numMoved = 0;
	    	
	    	for(int i = 0;i<initial.length;i++){ //I made a boolean array of size nxn to tell if a cell is unsatisfied or not
	    		for(int j = 0;j<initial[0].length;j++){
	    			if (isSatisfied(initial, i, j, threshold) != true)
	    				unsatisfied[i][j] = true;
	    		}
	    	}
	    	
	    	for(int i = 0;i<initial.length;i++){ //If a cell is unsatisfied, I check through the array for a blank spot and swap 
	    		unsat: for(int j = 0;j<initial[0].length;j++){//the blank spot's value with the unsatisfied cell
	    			optimizeXLocation(initial, optimalX, threshold);
	    			optimizeOLocation(initial, optimalO, threshold);
	    			
	    			if (unsatisfied[i][j]){
	    				if (initial[i][j] == 'X'){
		    				for(int a = 0;a<initial.length;a++){
		    					for(int b = 0;b<initial[0].length;b++){
		    						if (optimalX[a][b] == (true)){
		    							initial[a][b] = initial[i][j];
		    							initial[i][j] = ' ';
		    							numMoved++;
		    							continue unsat;
		    						}
		...
		public static void optimizeXLocation(char[][] initial, boolean[][] optimalX, int threshold){
			double same = 0;
			double total = 0;
...
public static void optimizeOLocation(char[][] initial, boolean[][] optimalO, int threshold){
	...
	for(int a = 0;a<initial.length;a++){
	    ...
	    double numMoved, int totalNumMoved){
	    	...
	    	for(int row = 0;row<tissue.length;row++){//add blank spaces to to dissatisfied cells
	    		for(int col = 0;col<tissue[0].length;col++){
	    			if (tissue[row][col] == ' ')
	    				numMoved++;
	    		}
